...
def split_train_val(images_dir: str, output_dir: str, lst_path: str):
    ...
    for image_name in lst:
        image_path: str = glob.glob(os.path.join(images_dir, "*", image_name), recursive=True)
        try:
            class_name: str = os.path.basename(os.path.dirname(image_path[0]))
            dest_dir: str = os.path.join(output_dir, class_name)
            os.makedirs(dest_dir, exist_ok=True)

            logging.info(f"Copying {image_path[0]} to {dest_dir}")
            shutil.copy(image_path[0], dest_dir)
        except:
            continue
...
if __name__ == "__main__":
    ...
    TSINGHUA_DOGS_ROOT_DIR: str = str(DATA_DIR / "New_TsinghuaDogs")
    ...
    # unzip_command: str = f"""
    #     unzip {DATA_DIR}/TsinghuaDogs_final.zip -d {TSINGHUA_DOGS_ROOT_DIR}
    # """
    ...
    # subprocess.run(f"unzip jar xvf {annotations_path} -d {TSINGHUA_DOGS_ROOT_DIR}", shell=True)
    ...
    # Prepare train set
    images_dir: str = "high-resolution"
    train_lst_path: str = os.path.join(DATA_DIR, "train_small.txt")
    train_dir: str = os.path.join(str(DATA_DIR / TSINGHUA_DOGS_ROOT_DIR), "train")
    os.makedirs(train_dir, exist_ok=True)
    split_train_val(images_dir, output_dir=train_dir, lst_path=train_lst_path)

    # Prepare validation set
    val_lst_path: str = os.path.join(DATA_DIR, "val_small.txt")
    val_dir: str = os.path.join(str(DATA_DIR / TSINGHUA_DOGS_ROOT_DIR), "val")
